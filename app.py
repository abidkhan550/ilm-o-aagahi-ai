...
# سسٹم کی ڈیفالٹ انکوڈنگ کو UTF-8 پر سیٹ کرنا
...
# ۲. CSS اسٹائلنگ (وہی پرانی اسٹائلنگ جو آپ کی تھی)
st.markdown("""
    <style>
    ...
    .header-container {
        ...
        background: rgba(255, 255, 255, 0.95); padding: 18px; border-radius: 16px;
        box-shadow: 0px 8px 32px rgba(0, 0, 0, 0.3); margin-top: 15px; margin-bottom: 25px;
    }
    .profile-img { width: 80px; height: 80px; border-radius: 50%; object-fit: cover; border: 3px solid #1e88e5; }
    .web-title { margin: 0; font-weight: 900 !important; font-size: 32px !important; font-family: 'Montserrat', sans-serif !important; background: linear-gradient(135deg, #0d47a1 0%, #00d2ff 100%); -webkit-background-clip: text; -webkit-text-fill-color: transparent; }
    .signature-text { margin: 0; color: #1e88e5 !important; font-size: 32px !important; font-family: 'Great Vibes', cursive !important; font-weight: normal !important; margin-top: -5px; }
    .stTextArea textarea { background-color: #ffffff !important; color: #111111 !important; border-radius: 14px !important; border: 2px solid #1e88e5 !important; font-size: 19px !important; }
    .stButton>button { width: 100%; background: linear-gradient(90deg, #1e88e5 0%, #1565c0 100%); color: white !important; font-weight: bold; font-size: 18px; border-radius: 12px; padding: 12px; border: none; margin-top: 10px; }
    </style>
...
with col_head2:
    target_language = st.selectbox("Language", ["English", "Urdu", "Pashto", "Arabic", "Hindi", "Spanish", "French"])

# ۴. ہیڈر
img_html = f'<img src="data:image/jpeg;base64,{img_base64}" class="profile-img">' if img_base64 else '<div style="font-size: 50px;">🎓</div>'
st.markdown(f'''
...
# ۵. API Key کنفیگریشن (یہاں آپ کی AQ والی کی بالکل ٹھیک کام کرے گی)
try:
    API_KEY = "AQ.Ab8RN6Igrs9C9adMBKQTeGi13QHqkIsb0ljPfFwsgUwS3o3j4g"
...
if st.button("🚀 Ask AI"):
    if not is_configured:
        st.error("Gemini is not initialized properly.")
